...
def evaluation_summary(days: int = Query(default=7, ge=1, le=90), db: Session = Depends(get_db)):
...
def list_admin_chat_logs(limit: int = Query(default=50, ge=1, le=200), db: Session = Depends(get_db)):
...
def metrics_overview(days: int = Query(default=7, ge=1, le=90), db: Session = Depends(get_db)):
    ...
    intent_rows = db.query(ChatLog.intent, func.count(ChatLog.id)).filter(ChatLog.created_at >= since).group_by(ChatLog.intent).all()
    conversation_count = db.query(func.count(Conversation.id)).filter(Conversation.updated_at >= since).scalar() or 0
    human_transfers = db.query(func.count(Conversation.id)).filter(Conversation.updated_at >= since, Conversation.handoff_status != "NONE").scalar() or 0
...
def metrics_daily(db: Session = Depends(get_db)):
