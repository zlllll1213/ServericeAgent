...
def get_admin_conversation(conversation_id: str, db: Session = Depends(get_db)):
...
def assign_conversation(conversation_id: str, request: AssignRequest, db: Session = Depends(get_db)):
    conversation = db.query(Conversation).filter(Conversation.conversation_id == conversation_id).first()
    if conversation is None:
        not_found("会话")
...
def reply_conversation(conversation_id: str, request: HumanReplyRequest, db: Session = Depends(get_db)):
    conversation = db.query(Conversation).filter(Conversation.conversation_id == conversation_id).first()
    if conversation is None:
        not_found("会话")
...
def resolve_conversation(conversation_id: str, request: ResolveRequest, db: Session = Depends(get_db)):
    conversation = db.query(Conversation).filter(Conversation.conversation_id == conversation_id).first()
    if conversation is None:
        not_found("会话")
