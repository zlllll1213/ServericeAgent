...
def list_knowledge_documents(knowledge_base: str | None = None, status: str | None = None, keyword: str | None = None, db: Session = Depends(get_db)):
...
def create_knowledge_document(request: KnowledgeDocumentCreate, db: Session = Depends(get_db)):
    now = datetime.now()
...
def update_knowledge_document(doc_id: str, request: KnowledgeDocumentUpdate, db: Session = Depends(get_db)):
...
def publish_knowledge_document(doc_id: str, db: Session = Depends(get_db)):
    doc = db.query(KnowledgeDocument).filter(KnowledgeDocument.doc_id == doc_id).first()
    if doc is None:
        not_found("知识库文档")
...
def archive_knowledge_document(doc_id: str, db: Session = Depends(get_db)):
    doc = db.query(KnowledgeDocument).filter(KnowledgeDocument.doc_id == doc_id).first()
    if doc is None:
        not_found("知识库文档")
    doc.status = "ARCHIVED"
