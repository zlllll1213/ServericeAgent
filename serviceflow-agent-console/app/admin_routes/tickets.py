...
def list_admin_tickets(status: str | None = None, priority: str | None = None, assigned_agent_id: str | None = None, db: Session = Depends(get_db)):
...
def get_admin_ticket(ticket_id: str, db: Session = Depends(get_db)):
...
def assign_ticket(ticket_id: str, request: AssignRequest, db: Session = Depends(get_db)):
    ticket = db.query(Ticket).filter(Ticket.ticket_id == ticket_id).first()
    if ticket is None:
        not_found("工单")
...
def resolve_ticket(ticket_id: str, request: ResolveRequest, db: Session = Depends(get_db)):
    ticket = db.query(Ticket).filter(Ticket.ticket_id == ticket_id).first()
    if ticket is None:
        not_found("工单")
